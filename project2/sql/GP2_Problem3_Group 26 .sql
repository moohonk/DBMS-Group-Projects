...
--Q1: Retrieve the names of all gym leaders along with their monologues at their respective gyms
SELECT first_name, gym_leader_monologue
FROM Gym LEFT JOIN Trainer ON Trainer.trainer_id = gym_leader_id;

--Q2: Retrieve the count of trainers who have completed the Pewter City gym
SELECT COUNT(*)
FROM GymBattle
WHERE city_name='Pewter City'

--Q3: Retrieve the average level of each trainer's pokemon team
SELECT trainer_id, AVG(level) AS average
FROM Pokemon
GROUP BY trainer_id;

--Q4: Retrieve a list of all trainers who have completed at least one gym, along with their highest level pokemon
SELECT trainer_id, MAX(level) FROM Pokemon
WHERE Pokemon.trainer_id IN (SELECT DISTINCT(trainer_id) FROM GymBattle)
GROUP BY trainer_id;

--Q5: A new gym has opened in Pewter City. It specializes in bug type pokemon and will give away the Moth Badge. Botanist Lief will be the gym leader.
...
--Q6: The gym leader of Cinnabar Island has changed to a new trainer in the region. He claims his trainer Id is 9999
UPDATE Gym
